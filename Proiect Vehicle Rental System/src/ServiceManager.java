...
public class ServiceManager {
    ...
    public void efectueazaPlata(Rezervare rezervare) {
        ...
        for (RezervareVehicul rv : rezervare.getRezervareVehicule()) {
            Date start = rv.getStartDate();
            Date end = rv.getEndDate();

            long durataZile = (end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24);
            if (durataZile <= 0) durataZile = 1;

            double pret = rv.getVehicul().calculeazaPretInchiriere((int) durataZile);
            totalDePlata += pret;
        }

        double avans = rezervare.getAvans();
        double restPlata = totalDePlata - avans;

        System.out.println("Total de plată (fara avans): " + restPlata + " RON");
    }
